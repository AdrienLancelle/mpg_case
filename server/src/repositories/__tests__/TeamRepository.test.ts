...
describe('TeamRepository', () => {
  ...
  describe('updateTeamName', () => {
    it('should update team name successfully', async () => {
      ...
      // Act
      const result = await repository.updateTeamName(teamId, name);
      ...
      expect(result).toEqual({
        success: true,
        team: expect.objectContaining({name: name}),
      });
    });
    ...
    it('should throw error if team not found', async () => {
      ...
      // Act & Assert
      await expect(repository.updateTeamName(teamId, name)).rejects.toThrow(
        "Impossible de mettre à jour le nom de l'équipe",
      );
