...
describe('UserRepository', () => {
  ...
  describe('getUsersByLeagueId', () => {
    it('should return users for a league', async () => {
      ...
      expect(result).toEqual([{name: 'User One'}, {name: 'User Two'}]);
    ...
    it('should return empty array if league has no users', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(`Retrieved 0 users for league: ${leagueId}`);
    });
    ...
    it('should return empty array if league not found', async () => {
      ...
      // Assert
      expect(result).toEqual([]);
      expect(logger.error).toHaveBeenCalledWith('Error retrieving users:', expect.any(Error));
    });
