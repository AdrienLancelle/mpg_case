...
describe('LeagueRepository', () => {
    ...
    describe('createLeague', () => {
        it('should create a league successfully', async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith(`League created successfully with ID: ${command.id}`);
        });
    ...
    describe('updateTeamName', () => {
        it('should update team name successfully', async () => {
            ...
            expect(mockCollection.upsert).toHaveBeenCalledWith(
                command.teamId,
                expect.objectContaining({ name: command.newName })
            );
            expect(logger.info).toHaveBeenCalledWith(`Team name updated successfully for ID: ${command.teamId}`);
