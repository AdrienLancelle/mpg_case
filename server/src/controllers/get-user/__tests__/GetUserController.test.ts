...
describe('GetUserController', () => {
  ...
  it('should get users successfully', async () => {
    // Arrange
    const expectedUsers = [{name: 'User One'}, {name: 'User Two'}];
    mockHandler.handle.mockResolvedValueOnce(expectedUsers);

    // Act
    const response = await request(app).get(`/api/leagues/${leagueId}/users`);

    // Assert
    expect(response.status).toBe(200);
    expect(response.body).toEqual({users: expectedUsers});
    expect(mockHandler.handle).toHaveBeenCalledWith(expect.objectContaining({leagueId}));
